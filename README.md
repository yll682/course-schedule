...
| 多用户 | 每人独立课表，互不干扰 |
| 深色模式 | 跟随系统自动切换 |
| 公告栏 | 管理员可分别编辑顶部横幅与弹窗两套富文本公告，各自独立启用、限制展示次数、设置是否可关闭，另有预留广告位；用户可在设置页查看并恢复已关闭的公告 |

### 管理员专属

- 调整全局后台抓取间隔
- 配置公告栏：顶部横幅与弹窗两套独立公告，各自有富文本内容（支持部分 HTML，服务端白名单过滤）、每位用户展示次数、是否允许关闭；广告位（预留占位，可填入 HTML 片段并选择显示在横幅上方、横幅下方、横幅内部或弹窗内部）
- 查看并撤销所有用户的分享码（含创建者、时长、周次等详情）
- 查看所有用户已缓存的课表（按周次点击，不触发实时请求）
- 两级管理员：`ADMIN_USERS` 中的学号为超级管理员（原有管理员自动升级），可在后台添加 / 移除普通管理员；除此之外两者权限相同，增删后立即生效无需重新登录
...
**超级管理员学号**：安装时填写，或事后修改 `.env` 中的 `ADMIN_USERS`（逗号分隔可填多个）。普通管理员无需改配置，由超级管理员在设置页「管理后台 → 管理员」中添加。
